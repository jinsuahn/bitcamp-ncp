...
public class ServerApp {
	...
	public ServerApp() throws Exception {
		...
		//
		// 3) Mybatis 설정 파일을 읽을 때 사용할 입력 스트림 개체 준비
		InputStream mybatisConfigInputStream = Resources.getResourceAsStream("bitcamp/myapp/mapper/BoardMapper.xml");
		//
		// 4) SqlSessionFactoryBuilder 준비
		SqlSessionFactoryBuilder builder = new SqlSessionFactoryBuilder();

		// 5) builder를 이용하여 SqulSessionFactory 객체 생성
	...
	private void processRequest(StreamTool streamTool) throws Exception {
		...
		loop: while (true) {
			...
			try {
				...
				streamTool.println("메뉴 번호가 옳지 않습니다!")
				.println()
				.send();
				continue;
			}
			...
			try {
				...
				streamTool.printf("명령 실행 중 오류 발생! - %s : %s\n", e.getMessage(), e.getClass().getSimpleName()).send();
			}
		}
	...
	void menu(StreamTool streamTool) throws Exception {
		streamTool.println("1. 학생관리").println("2. 강사관리").println("3. 게시판").println("4. 인사").println("9. 종료")
				.println("메뉴 번호:").send();
	}
